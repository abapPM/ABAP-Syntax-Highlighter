...
CLASS zcl_highlighter_factory IMPLEMENTATION.
  ...
  METHOD create.
    ...
    IF iv_filename CP '*.abap'.
      CREATE OBJECT ro_instance TYPE zcl_highlighter_abap.
    ELSEIF iv_filename CP '*.xml' OR iv_filename CP '*.html'.
      CREATE OBJECT ro_instance TYPE zcl_highlighter_xml.
    ELSEIF iv_filename CP '*.css'.
      CREATE OBJECT ro_instance TYPE zcl_highlighter_css.
    ELSEIF iv_filename CP '*.js'.
      CREATE OBJECT ro_instance TYPE zcl_highlighter_js.
    ELSEIF iv_filename CP '*.json' OR iv_filename CP '*.jsonc'.
      CREATE OBJECT ro_instance TYPE zcl_highlighter_json.
    ELSEIF iv_filename CP '*.txt' OR iv_filename CP '*.ini'  OR iv_filename CP '*.text'.
      CREATE OBJECT ro_instance TYPE zcl_highlighter_txt.
    ELSEIF iv_filename CP '*.md' OR iv_filename CP '*.markdown'.
      CREATE OBJECT ro_instance TYPE zcl_highlighter_md.
    ELSEIF iv_filename CP '*.diff'.
      CREATE OBJECT ro_instance TYPE zcl_highlighter_diff.
    ELSE.
      CLEAR ro_instance.
    ENDIF.
